...
import Ingredient from '../add-ingr/add-ingr.js'
const ID_DOM_EL = {
...
class CategoryIngredients {
  constructor(renderThisDomElement) {
    ...
    localStorage.setItem('INGRIDIENTS',JSON.stringify(data))
  ...
  elementHeader = () => {
    ...
    <section class="row yesh__section-header">
        ...
        <div class="col s3 m2 l1 yesh__header-btn">
            <button id="add-ingr"  class="yesh__btn-add">Добавить</button>
        </div>
    </section>
      `;
  };
  ...
  // get fetch
  getDataByFetch = () => {
    this.data = JSON.parse(localStorage.getItem('INGRIDIENTS'));
  ...
  postFetchDeleteIngredient = id => {
    ...
    localStorage.setItem('INGRIDIENTS',JSON.stringify(this.data))
  };
  deleteCategory = e => {
    const { id } = e.target;
