...
class addDish {
  ...
  interactive = () => {
    ...
    btnAdd.addEventListener('click', e => {
      ...
      const alreadyUsed = arr.find(el => el.name === inputName.value.trim());
      if(alreadyUsed){
        alert(`Категория ${newDish.name.trim()} уже используется, выберите другое название!`);
        return;
      }
      if (inputImg.value !== '') {
