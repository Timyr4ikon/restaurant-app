...
class Dishes {
  ...
  createContentHeader = () => {
    ...
    root.innerHTML =
      '<div class="content-header"><h2 class="content-header__title">Категории блюд <span class="content-header__quantity" data-action="quantity">0</span></h2><button class="content-header__button" data-action="add">Добавить</button></div>';
  };
  createFastSearch = () => {
    const root = document.querySelector('main');
    root.innerHTML +=
  ...
  createDishesItems = () => {
    ...
    arrayFromBackend.forEach(el => {
      ...
      categoriesItem.id = el.id;
      categoriesItem.classList.add('categories__item');
      const par = `<div class="categories__meal"><img  class="categories__img" 
      src="${el.img}" alt=""><span data-action="name">${el.name}</span></div><div class="btn-open">
     <div class="btn-open__btnn" data-action="openHideMenu">...<ul class="btn-open__list"><li class="btn-open__item" data-action="hide" >
      Скрыть</li><li class="btn-open__item" data-action="delete">Удалить</li></ul></div></div>`;
    ...
    list.addEventListener('click', e => {
      ...
      if (element.dataset.action === 'delete') {
        if (confirm('Are you shuuuure, ? =3')) {
          const allDishes = JSON.parse(localStorage.getItem('DISHES'));
          const filteredDishes = allDishes.filter(el => el.id !== element.parentNode.parentNode.parentNode.parentNode.id );
