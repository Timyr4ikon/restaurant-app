...
class PaymentModule {
                ...
                <form class="checkout__section">
                    ...
                    <div class="checkout__close-navbar">
                        <span class="checkout__link--alert">
                            <a class="close-link" href="https://www.google.com/search?rlz=1C1PRFI_enUA885UA885&sxsrf=ALeKk032WfMN_0uUYSE9TkNZSJVYW6CLew%3A1597529607118&ei=B144X4bmBu6FrwSpuJLYBg&q=%D0%BA%D0%B0%D0%BA+%D0%BD%D0%B5+%D0%B1%D1%8B%D1%82%D1%8C+%D0%BF%D0%B0%D1%86%D1%8E%D0%BA%D0%BE%D0%BC&oq=%D0%BA%D0%B0%D0%BA+%D0%BD%D0%B5+%D0%B1%D1%8B%D1%82%D1%8C+%D0%BF%D0%B0%D1%86%D1%8E%D0%BA%D0%BE%D0%BC&gs_lcp=CgZwc3ktYWIQAzIFCCEQoAE6BAgjECc6BAguEEM6BAgAEEM6BQgAELEDOgIIADoFCC4QsQM6AgguOggIABCxAxCDAToHCAAQsQMQQzoFCAAQywE6BggAEBYQHjoICAAQFhAKEB5Q-hRYuUlgqktoAHAAeACAAYYBiAHSDpIBBDIuMTWYAQCgAQGqAQdnd3Mtd2l6wAEB&sclient=psy-ab&ved=0ahUKEwjGhYDGnZ7rAhXuwosKHSmcBGsQ4dUDCAw&uact=5" target="_blank" >Не платить</a></span>
                        <button class="checkout__pay-button btn-small type="submit" id="pay">Оплатить</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    submit () {
        ...
        const cardPay = document.querySelector('#sum-card');
        let restSum = document.querySelector('.checkout__rest-value')
        submitBtn.addEventListener('click',(e) => {
            e.preventDefault()
            if(cardPay.textContent !== ''){
                alert('Сейчас в ваше окно залетит терминал))');
                return new LoginForm()
            }
            if(restSum.textContent !== '0'){
                 alert("Сдача официанту, хорошего дня)");
                return new LoginForm()

            }
            else{
                 alert('Я надеюсь вы помните об официанте и кинете ему пару сотен)')
                new LoginForm()
    ...
    calculate() {
        ...
        for (var idx = 0; idx < number.length; idx++) {
            ...
            number[idx].addEventListener("click", function(e) {
                ...
                if(restSum.textContent === "NaN"){
                    restSum.textContent = 'Упс)'
                }

                checkoutPayButton.classList.remove('active-pay-button')
                if(Number(checkoutSum.textContent)=== getNumber()){
                    checkoutPayButton.classList.add('active-pay-button')
                    checkoutPayButton.disabled = false;
                    return;
                }
